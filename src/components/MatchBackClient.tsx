...
const App: React.FC = () => {
  ...
  const findMatches = () => {
    ...
    const matchBackArray: CSVRow[] = matchBack.filter((mbRow) => {
      return list.some((listRow) => {
        const listValue = listRow[matchField]?.trim().toLowerCase();
        const matchBackValue = mbRow[matchField]?.trim().toLowerCase();
        return listValue === matchBackValue;
      });
    });

    if (sortField) {
      matchBackArray.sort((a, b) =>
        ("" + a[sortField]).localeCompare("" + b[sortField])
      );
    }
    if (matchBackArray.length === 0) {
      setErrorMessage(
        "No matches found. Make sure the match field is the same in both files."
      );
    } else {
  ...
  return (
    <>
      <div className=" flex flex-col items-center justify-center p-5 bg-white shadow rounded-lg w-full h-screen">
        <div className="p-5">
          ...
          <p className="text-center">
            Upload your list and matchback files to find matches. Sort by any
            given key
          </p>
        </div>
        <div className="grid grid-col-3 justify-center">
          <input
            type="file"
            onChange={(e) => handleFileChange(e, setList)}
            className="p-5"
          />
          <input
            type="file"
            onChange={(e) => handleFileChange(e, setMatchBack)}
            className="p-5"
          />
          {headers.length > 0 && (
      ...
      <div className="p-5 w-full overflow-scroll h-20">
        {" "}
